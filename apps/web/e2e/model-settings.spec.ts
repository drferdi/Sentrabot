...
const LOCAL_MODEL_ID = "sentrabot-e2e-local";
...
test("connects, lists, and uses an OpenAI-compatible endpoint", async ({ page }, testInfo) => {
  ...
  try {
    ...
    await signup(page, `local-model-${stamp}@sentrabot.test`, "password12", userName);
...
test("model settings connect, replace, and cancel provider authentication", async ({ page }) => {
  ...
  await signup(page, `models-${stamp}@sentrabot.test`, "password12", userName);
