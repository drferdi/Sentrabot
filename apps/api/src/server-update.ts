...
import { ServerUpdateRunSchema } from "@sentrabot/contracts";
import {
...
export interface UpdaterProxyConfig {
  ...
  /** Current `SENTRABOT_IMAGE_TAG` when known; selects compose pull vs rebuild commands. */
...
export async function readServerUpdateStatus(
  ...
  const imageTagHint = config.imageTag?.trim() || process.env.SENTRABOT_IMAGE_TAG?.trim() || null;
