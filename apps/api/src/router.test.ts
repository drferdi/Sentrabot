...
describe("account preferences", () => {
  function preferencesDeps(avatarStyle: string) {
    ...
    const deps = {
      ...
      dataDir: "/tmp/sentrabot-router-test",
  ...
  it("coerces unknown stored avatar styles to clay on me", async () => {
    ...
    expect(response.status).toBe(200);
    await expect(response.json()).resolves.toEqual({
      json: expect.objectContaining({ avatarStyle: "clay" }),
    });
  });
});
