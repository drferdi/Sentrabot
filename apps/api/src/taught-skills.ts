...
import type { AgentHomeStore, JobPublisher, SandboxProvider } from "@sentrabot/adapter-kit";
import {
  type AdapterContext,
  runContinueJob,
...
import type { Actor, MessageBlock, TaughtSkill } from "@sentrabot/contracts";
import {
  ACTIVE_RUN_STATUSES,
...
import { IsolationError, type PrismaClient, type ThreadEvents } from "@sentrabot/db";
...
async function ensureGraphicalComputer(
  deps: TaughtSkillsDeps,
  actor: Actor,
  bot: Awaited<ReturnType<ReturnType<typeof import("@sentrabot/db").createRepos>["getBot"]>>,
) {
