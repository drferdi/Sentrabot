...
const nativeNotifications =
  Platform.OS === "android"
    ? requireNativeModule<NativeNotifications>("SentraBotNotifications")
    : null;
...
export function notificationTargetsThread(
  ...
  const dataThreadId = data.threadId ?? data["sentrabot.threadId"];
  if (target.threadId && dataThreadId) return dataThreadId === target.threadId;
  return Boolean(
    target.botId && (data.botId === target.botId || data["sentrabot.botId"] === target.botId),
  );
}
