...
const ENDPOINT_KEY = "sentrabot.api_base";
...
export async function signIn(email: string, password: string) {
  const res = await fetch(`${currentApiBase()}/api/auth/sign-in/email`, {
    method: "POST",
    headers: { "content-type": "application/json", origin: "sentrabot://" },
...
export async function signOut() {
  ...
  await fetch(`${currentApiBase()}/api/auth/sign-out`, {
    method: "POST",
    headers: { "content-type": "application/json", origin: "sentrabot://", ...headers },
  }).catch(() => undefined);
