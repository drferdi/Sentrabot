...
describe("Android mobile platform contract", () => {
  ...
  it("requests live-update promotion and exposes its Android settings", () => {
    const nativeRoot = resolve(
      mobileRoot,
      "modules/sentrabot-notifications/android/src/main/java/com/sentrabot/notifications",
    );
    const service = readFileSync(resolve(nativeRoot, "SentraBotNotificationService.kt"), "utf8");
    const module = readFileSync(resolve(nativeRoot, "SentraBotNotificationsModule.kt"), "utf8");
    ...
    expect(module).toContain("SentraBotNotificationService.clearSession(context)");
    ...
    expect(service).toContain('"sentrabot://group-thread?groupId=');
  ...
  it("shows live updates only for working runs and ties the pill to a real bot", () => {
    const service = readFileSync(
      resolve(
        mobileRoot,
        "modules/sentrabot-notifications/android/src/main/java/com/sentrabot/notifications/SentraBotNotificationService.kt",
      ),
      "utf8",
    );
    expect(service).toContain(
    ...
    expect(service).toContain('putString("sentrabot.botId", run.botId)');
