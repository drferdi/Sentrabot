...
describe("normalizeApiBase", () => {
  it("trims, adds https, and keeps only the origin", () => {
    ...
    expect(normalizeApiBase("https://sentrabot.example.com:8443/api/")).toEqual({
      ok: true,
      url: "https://sentrabot.example.com:8443",
    });
...
describe("display and warnings", () => {
  ...
  it("shows host and non-default port", () => {
    expect(displayApiHost("https://sentrabot.example.com")).toBe("sentrabot.example.com");
  ...
  it("treats the compile-time default as not custom", () => {
    ...
    expect(usesCustomApiBase("https://sentrabot.example.com")).toBe(true);
  });
});

describe("probeApiBase", () => {
  it("accepts a Sentra Bot /rpc/health response", async () => {
  ...
  it("rejects a host that is up but is not Sentra Bot", async () => {
    const fetchImpl = vi.fn(
