package com.sentrabot.notifications
...
class SentraBotNotificationService : Service() {
  ...
  private fun post(run: RunRecord, copy: NotificationCopy) {
    ...
    val notification = builder(copy.channel)
      .setSmallIcon(R.drawable.ic_sentrabot_notification)
      .setContentTitle(copy.title)
      .setContentText(copy.body)
      .setStyle(Notification.BigTextStyle().bigText(copy.body))
      .setContentIntent(openApp(run))
      .addExtras(Bundle().apply {
        putString("sentrabot.botId", run.botId)
        putString("sentrabot.threadId", run.threadId)
      })
      .setAutoCancel(true)
  ...
  private fun liveStatusIcon(run: RunRecord, avatarStyle: String): Icon {
    if (avatarStyle != "organic") {
      return Icon.createWithResource(this, R.drawable.ic_sentrabot_notification)
    }
  ...
  private fun openApp(run: RunRecord? = null): PendingIntent {
    val intent = if (run == null) {
      packageManager.getLaunchIntentForPackage(packageName) ?: Intent(Intent.ACTION_VIEW, Uri.parse("sentrabot://"))
    } else {
      val destination = if (run.groupId != null) {
        "sentrabot://group-thread?groupId=${Uri.encode(run.groupId)}&name=${Uri.encode(run.groupName.orEmpty())}"
      } else {
        "sentrabot://thread?botId=${Uri.encode(run.botId)}&name=${Uri.encode(run.botName)}"
      }
      Intent(
        Intent.ACTION_VIEW,
  ...
  companion object {
    ...
    private const val ACTION_THREAD_CHANGED = "com.sentrabot.notifications.THREAD_CHANGED"
    private const val PROMOTED_ONGOING_EXTRA = "android.requestPromotedOngoing"
    private const val POLL_INTERVAL_MS = 8_000L
    private const val STATE_PREFERENCES = "com.sentrabot.notification_state"
    ...
    fun start(context: Context) {
      val intent = Intent(context, SentraBotNotificationService::class.java)
      context.startService(intent)
    }

    fun stop(context: Context) {
      context.stopService(Intent(context, SentraBotNotificationService::class.java))
    }
    ...
    fun setOpenThread(context: Context, botId: String?, threadId: String?) {
      ...
      context.startService(
        Intent(context, SentraBotNotificationService::class.java).setAction(ACTION_THREAD_CHANGED),
      )
    }
...
private object Channels {
  const val LIVE = "sentrabot_live"
  const val MESSAGES = "sentrabot_messages"
  const val SCHEDULED = "sentrabot_scheduled"
  const val ATTENTION = "sentrabot_attention"
}
...
private fun rpc(endpoint: String, token: String, procedure: String, input: JSONObject): JSONObject {
  ...
  return try {
    ...
    connection.setRequestProperty("Origin", "sentrabot://")
