...
class SentraBotNotificationsModule : Module() {
  override fun definition() = ModuleDefinition {
    Name("SentraBotNotifications")
    ...
    AsyncFunction("setSettings") { value: Map<String, Boolean>, endpoint: String, token: String ->
      ...
      if (storage.settings.liveConnection && endpoint.isNotBlank() && token.isNotBlank()) {
        SentraBotNotificationService.start(context)
      } else {
        SentraBotNotificationService.stop(context)
      }
    }
    ...
    AsyncFunction("stop") { clearSession: Boolean ->
      val context = context()
      if (clearSession) {
        SentraBotNotificationService.clearSession(context)
    ...
    AsyncFunction("setOpenThread") { botId: String?, threadId: String? ->
      SentraBotNotificationService.setOpenThread(context(), botId, threadId)
    }
