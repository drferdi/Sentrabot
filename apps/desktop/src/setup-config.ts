...
import type { DesktopSetup } from "@sentrabot/contracts";

/** Where `pnpm dev` serves the Sentra Bot web app on this machine. */
...
/**
 * Accepts what a person would actually type ("localhost:5173", "sentrabot.example.com")
 * and returns a canonical http(s) origin, or null when the input can never
 * securely address a Sentra Bot server.
 */
export function normalizeServerUrl(input: string): string | null {
  ...
  // Sentra Bot serves its renderer, RPC, and auth routes from one origin. Keeping a
 ...
 * `SENTRABOT_WEB_URL` still wins over saved configuration so test and performance
...
/** Each Sentra Bot origin gets its own persistent cookie and storage partition. */
export function sessionPartitionForServerUrl(targetUrl: string): string | null {
  try {
    ...
    return `persist:sentrabot-${digest}`;
  } catch {
    return null;
  }
}
...
export function isSentraBotHealth(value: unknown): boolean {
 ...
 * metadata endpoints. Cleartext HTTP to them is never a legitimate Sentra Bot
