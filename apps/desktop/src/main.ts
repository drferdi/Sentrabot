...
const PERFORMANCE_USER_DATA = process.env.SENTRABOT_PERFORMANCE_USER_DATA;
...
const updaterEnvironment = {
  ...
  disabled: process.env.SENTRABOT_DISABLE_AUTO_UPDATE === "1",
};
...
function createWindow(url: string, partition: string | null) {
  ...
  win.on("close", (event) => {
    if (
      ...
      !quitting &&
      process.env.SENTRABOT_DISABLE_WARM_WINDOW !== "1"
    ) {
      event.preventDefault();
      win.hide();
 ...
 * fixtures omit the Sentra Bot app-state marker.
 */
async function waitForMountedAppDocument(contents: Electron.WebContents) {
  ...
  while (Date.now() < deadline) {
    ...
    const ready = (await contents.executeJavaScript(`(() => {
      const appState =
        document.querySelector("[data-sentrabot-app-state]")?.getAttribute("data-sentrabot-app-state") ??
        null;
      ...
      // Desktop e2e fixtures mount a plain page without Sentra Bot app-state markers.
      if (appState === null) {
...
async function installBundledRenderer(
  ...
  if (!app.isPackaged || process.env.SENTRABOT_DISABLE_BUNDLED_RENDERER === "1") return;
...
function installApplicationMenu() {
  const changeServer: Electron.MenuItemConstructorOptions = {
    id: "change-sentrabot-server",
    label: "Change Sentra Bot Server…",
...
/** Setup IPC must only answer the setup window, never a connected Sentra Bot server. */
  ...
  try {
    ...
    if (response.status >= 300 && response.status < 400) {
      return {
        ...
        url,
        error: "That address redirects elsewhere. Enter the final Sentra Bot server address.",
      };
    }
    if (!response.ok) {
      return {
    ...
    if (!isSentraBotHealth(health)) {
      return {
        ok: false,
        status: response.status,
        url,
        error: "That address did not respond like a Sentra Bot server.",
      };
    }
    return {
      ok: true,
...
app.whenReady().then(async () => {
  ...
  const target = resolveStartupTarget({
    envUrl: process.env.SENTRABOT_WEB_URL,
    saved: currentSetup,
    forceSetup: process.env.SENTRABOT_FORCE_SETUP === "1",
  });
  if (process.env.SENTRABOT_PERFORMANCE_CLEAR_CACHE === "1") {
