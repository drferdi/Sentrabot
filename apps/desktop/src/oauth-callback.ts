import type { SentraBotDesktopOAuthCallback } from "@sentrabot/contracts";
 ...
 * in the redirect URL. Sentra Bot runs no listener on that port and asks for the
...
): SentraBotDesktopOAuthCallback | undefined {
  let target: URL;
  try {
    target = new URL(url);
  } catch {
