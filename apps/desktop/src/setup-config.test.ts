...
describe("server address normalization", () => {
  it("assumes http locally and https for a bare public host", () => {
    ...
    expect(normalizeServerUrl("sentrabot.example.com")).toBe("https://sentrabot.example.com");
  });
  ...
  it("rejects cleartext public servers but permits private-network development", () => {
    expect(normalizeServerUrl("http://sentrabot.example.com")).toBeNull();
  ...
  it("rejects cleartext link-local addresses used by cloud metadata endpoints", () => {
    ...
    // HTTPS to link-local still normalizes; the health probe must match Sentra Bot.
  ...
  it.each(["", "   ", "not a url", "ftp://example.com", "file:///etc/passwd", "http://"])(
    "rejects an address that cannot reach a Sentra Bot server (%s)",
    (value) => {
  ...
  it("rejects embedded credentials rather than writing them to disk", () => {
    expect(normalizeServerUrl("https://user:secret@sentrabot.example.com")).toBeNull();
  });
});

describe("saved setup", () => {
  it("round-trips through the on-disk format", () => {
    const setup = { mode: "existing", serverUrl: "https://sentrabot.example.com" } as const;
...
describe("startup target", () => {
  const saved = { mode: "existing", serverUrl: "https://sentrabot.example.com" } as const;
  ...
  it("opens the saved instance on later launches", () => {
    expect(resolveStartupTarget({ saved })).toEqual({
      kind: "app",
      url: "https://sentrabot.example.com",
      source: "saved",
    });
  });

  it("lets SENTRABOT_WEB_URL point the shell anywhere without touching saved setup", () => {
  ...
  it("ignores an empty SENTRABOT_WEB_URL", () => {
...
describe("bundled renderer eligibility", () => {
  it("stands in for http(s) origins only", () => {
    ...
    expect(servesBundledRenderer("https://sentrabot.example.com")).toBe(true);
...
describe("remote-content isolation", () => {
  it("uses a stable, opaque session partition per server origin", () => {
    ...
    expect(first).toMatch(/^persist:sentrabot-[a-f0-9]{24}$/);
...
describe("Sentra Bot health response", () => {
  it("requires the public RPC health contract", () => {
    expect(isSentraBotHealth({ json: { ok: true, version: "0.1.0" } })).toBe(true);
    expect(isSentraBotHealth({ json: { ok: true } })).toBe(false);
    expect(isSentraBotHealth({ ok: true, version: "0.1.0" })).toBe(false);
  });
});
