...
Pull Postgres and `ghcr.io/drferdii/sentrabot/app` into any empty folder. No clone or image build.
Requires Docker Engine, the Compose plugin, curl, and OpenSSL.

```bash
mkdir -p sentrabot && cd sentrabot &&
curl -fsSLO https://raw.githubusercontent.com/drferdii/sentrabot/main/infra/compose/install-images.sh &&
...
(from the app image, on the internal network only) and pulls `ghcr.io/drferdii/sentrabot/computer`.
...
`SENTRABOT_IMAGE_TAG` and `SENTRABOT_COMPUTER_IMAGE_TAG` to the same published multi-arch release tag
when one exists (see [Published images and tags](#published-images-and-tags)). Changing only
`SENTRABOT_IMAGE_TAG` leaves the computer service on amd64-only `edge`. Do not assume `latest` is
published.
...
Compose runs Postgres, the sandbox supervisor (Docker socket), API, worker, and a Vite preview of the web app. Bot computers are sibling containers (`sentrabot/computer:local`) on separate per-bot networks; only the supervisor and screen proxy join each one. The API process does not get an unrestricted Docker socket; the supervisor owns the lifecycle.
...
```env
SENTRABOT_LOCAL_MODELS=qwen3:4b,llama3.1:8b
SENTRABOT_LOCAL_MODELS_URL=http://127.0.0.1:11434/v1
SENTRABOT_LOCAL_CONTEXT_WINDOW=32768
SENTRABOT_LOCAL_MAX_TOKENS=4096
```

The loopback default is suitable when running Sentra Bot from a source checkout. In Docker Compose,
...
leave Sentra Bot through this URL. Leave `SENTRABOT_LOCAL_MODELS` blank to disable the provider.
...
the exact model id from that server, and an optional API key. By default Sentra Bot only allows
loopback, RFC1918, and `host.docker.internal` targets. To permit public hostnames, set
`SENTRABOT_OPENAI_COMPAT_ALLOW_PUBLIC=1` in the deployment environment. Public hostnames must resolve
...
- **Published images** (`docker-compose.images.yml`) default to `SANDBOX_PROVIDER=docker` with a
  local supervisor and published `ghcr.io/drferdii/sentrabot/computer` image. No E2B account required.
...
- **E2B** runs bot computers away from the Sentra Bot host and is a good choice for public or multi-user
  production deployments. Sentra Bot checkpoints the portable workspace and browser-profile directory to
...
- **Box by ASCII** provides a managed Linux desktop through `BOX_API_KEY` and optionally
  `BOX_API_URL`. Sentra Bot always creates or resumes boxes with `noEnv: true`, keeps the portable
...
2. Clone the repository on the VM and create a root `.env` with production-only values. At minimum set
   ...
   `SENTRABOT_HOST`, and the three public origins. Set `SENTRABOT_DEPLOY_DIR` when the checkout is not at
   the supported Linux default, `/srv/sentrabot`. Use URL-safe random values for database credentials.
   If you enable the `updater` profile, also set a dedicated `SENTRABOT_UPDATER_TOKEN` (at least 32
...
WAKEUP_DRIVER=graphile
DATA_DIR=/data
# Absolute path of this checkout as the Docker daemon sees it. /srv/sentrabot is the Linux default;
# set this explicitly for every other layout. See "The deploy directory must be one path" below.
SENTRABOT_DEPLOY_DIR=/srv/sentrabot
SENTRABOT_IMAGE_TAG=local
# Optional: required only with `--profile updater`.
# SENTRABOT_UPDATER_TOKEN=replace-with-32-plus-character-updater-token
```
...
**Build, do not pull, for a first deployment.** `SENTRABOT_IMAGE_TAG` ships as `local`, a tag no
...
`SENTRABOT_IMAGE_TAG` to that release tag and running `pull` followed by `up -d --wait --pull never`.
...
`/usr/local/sbin/sentrabot-backup` and enable the supplied `sentrabot-backup.timer`. It creates a verified
Postgres custom-format dump plus an application-data archive under `/var/backups/sentrabot`, with mode
...
`SENTRABOT_IMAGE_TAG` (or rebuild `local`) and `up -d --wait --pull never`.

The updater sidecar has its own image and tag so an update never recreates the process performing
it. Move it deliberately by setting `SENTRABOT_UPDATER_IMAGE_TAG` to the full `sha-<commit>` tag, then
...
`GIT_SHA=$(git rev-parse HEAD)`, run `pnpm --filter @sentrabot/db migrate`, then restart API and worker.
...
| Image | Contents |
| --- | --- |
| `ghcr.io/drferdii/sentrabot/app` | api, worker, web, and sandbox supervisor — one image, multiple commands |
| `ghcr.io/drferdii/sentrabot/computer` | Linux desktop used as each bot computer |
| `ghcr.io/drferdii/sentrabot/updater` | the updater sidecar, plus the Docker CLI |
...
also pull the same app tags once `SENTRABOT_IMAGE_TAG` is set to a published value.

If you deploy from your own fork, set `SENTRABOT_IMAGE` and `SENTRABOT_UPDATER_IMAGE` to your namespace —
...
`SENTRABOT_IMAGE_TAG` and `SENTRABOT_COMPUTER_IMAGE_TAG` to the same published release tag rather than
`edge`. Changing only `SENTRABOT_IMAGE_TAG` leaves the computer service on amd64-only `edge`. Until a
...
content addressing can pin `SENTRABOT_IMAGE` outside the automatic updater to a verified digest.
...
`SENTRABOT_UPDATER_TOKEN` and explicitly start the profile:
...
`SENTRABOT_UPDATER_TOKEN`. Operator CLI upgrades above do not need it; the sidecar is for automated
...
- *Official repository:* resolves the newest stable release and its source commit with
  ...
  outgoing tag in `SENTRABOT_IMAGE_TAG_PREVIOUS`, explicitly pulls the new image, then runs
...
- *Fork (Advanced):* a fork has no published images, so the sidecar fast-forwards the checkout in
  `SENTRABOT_DEPLOY_DIR` and runs `up -d --build`. This builds on the server and takes minutes rather
...
`SENTRABOT_DEPLOY_DIR` is bind-mounted into the updater at the same path it is read from
(`${SENTRABOT_DEPLOY_DIR}:${SENTRABOT_DEPLOY_DIR}`), and that is load-bearing rather than tidy. Production
Compose defaults both sides to `/srv/sentrabot`; set the variable for any other layout. When the
updater runs `docker compose -p <project> --file $SENTRABOT_DEPLOY_DIR/infra/compose/docker-compose.prod.yml up -d`,
...
`SENTRABOT_COMPOSE_PROJECT_NAME`; the final fallback is `sentrabot-prod`. Without that propagation, a
...
- **Linux.** The daemon shares the host filesystem, so the checkout path is the answer:
  `/srv/sentrabot` is the default and supported production layout. Set `SENTRABOT_DEPLOY_DIR` explicitly
  when the checkout is elsewhere.
- **Docker Desktop (Windows/macOS).** The daemon runs in a VM that mounts your drive somewhere else.
  On Windows, `C:` appears at `/run/desktop/mnt/host/c`, so a checkout at `C:\Users\you\sentrabot` is
  `SENTRABOT_DEPLOY_DIR=/run/desktop/mnt/host/c/Users/you/sentrabot`. Host Git may use `core.autocrlf=true`; the updater ignores CR-only diffs so that does not block `/apply`. Verify the mount before deploying:

```bash
docker compose --env-file .env -f infra/compose/docker-compose.prod.yml \
  --profile updater run --rm updater git -C "$SENTRABOT_DEPLOY_DIR" log --oneline -1
```
...
- The process environment carries only updater settings (`SENTRABOT_UPDATER_TOKEN`, deploy path,
...
Enabling the `updater` profile requires `SENTRABOT_UPDATER_TOKEN` to be a dedicated random value (at
...
## What “Sentra Bot Cloud” still needs
...
10. Turn on `SIGNUP_ALLOWLIST` until you want open registration. There is no Sentra Bot-managed model billing in version 1 — users bring keys.

Expo / desktop installers are clients of that origin (`EXPO_PUBLIC_API_URL`, `SENTRABOT_WEB_URL`). They are not a Cloud control plane.
