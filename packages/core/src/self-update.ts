/** The repository a stock Sentra Bot deployment tracks. */
export const OFFICIAL_REPO_URL = "https://github.com/drferdii/sentrabot";
...
/** `github.com/drferdii/sentrabot` for every spelling of the same remote, for comparison only. */
...
export function updateSteps(input: UpdatePlanInput): UpdateStep[] {
  ...
  steps.push(
    ...
    },
    {
      ...
      command: "pnpm",
      args: ["--filter", "@sentrabot/db", "run", "generate"],
    },
    {
      id: "build",
      label: "Build the web app",
      command: "pnpm",
      args: ["--filter", "@sentrabot/web", "run", "build"],
    },
    {
      id: "migrate",
      label: "Apply database migrations",
      command: "pnpm",
      args: ["--filter", "@sentrabot/db", "run", "migrate"],
    },
  );
  return steps;
}
...
export const RESTART_SUPERVISOR_ENV = "SENTRABOT_UPDATE_RESTART_SUPERVISOR";
...
export function restartSupervisorAdvice(supervisor: RestartSupervisor): string {
  if (supervisor.kind !== "none") {
    return `Sentra Bot will exit after updating and ${supervisor.label} will start it on the new code.`;
  }
  return `No process supervisor was detected, so Sentra Bot will not exit on its own. Restart the API, worker, and web processes yourself, or set ${RESTART_SUPERVISOR_ENV} to the supervisor that restarts them (for example "docker" with restart: unless-stopped, or run under systemd with Restart=always).`;
}
