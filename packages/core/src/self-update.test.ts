...
describe("normalizeRepoUrl", () => {
  it("accepts the official repository and https forks", () => {
    expect(normalizeRepoUrl(OFFICIAL_REPO_URL)).toEqual({
      url: "https://github.com/drferdii/sentrabot",
    });
    expect(normalizeRepoUrl("  https://github.com/me/sentrabot.git/  ")).toEqual({
      url: "https://github.com/me/sentrabot.git",
    });
    expect(normalizeRepoUrl("https://git.example.com:8443/team/sentrabot.git")).toEqual({
      url: "https://git.example.com:8443/team/sentrabot.git",
    });
  });

  it("accepts ssh remotes in both spellings", () => {
    expect(normalizeRepoUrl("git@github.com:me/sentrabot.git")).toEqual({
      url: "git@github.com:me/sentrabot.git",
    });
    expect(normalizeRepoUrl("ssh://git@github.com/me/sentrabot.git")).toEqual({
      url: "ssh://git@github.com/me/sentrabot.git",
    });
  });
  ...
  it("refuses transports and shapes that are not a git remote", () => {
    for (const input of [
      "",
      "   ",
      "http://github.com/me/sentrabot",
      "git://github.com/me/sentrabot",
      "file:///etc/passwd",
      "/srv/sentrabot",
      "https://github.com",
      "https://github.com/onlyone",
      "https://github.com/me/../../etc",
      "https://user:secret@github.com/me/sentrabot",
      "https://token@github.com/me/sentrabot",
      "ssh://bad%0auser@github.com/me/sentrabot",
      "https://github.com/me/sentrabot?x=1",
      "https://github.com/me/sentrabot#frag",
      "https://github.com/me/rak azo",
      "--upload-pack=touch /tmp/pwned",
      "-o ProxyCommand=id",
      `https://github.com/me/sentrabot${"x".repeat(400)}`,
    ]) {
  ...
  it("rejects an embedded newline that could smuggle a second argument", () => {
    expect(normalizeRepoUrl("https://github.com/me/sentrabot\n--exec=id")).toHaveProperty("error");
  });
});

describe("repoIdentity", () => {
  it("treats every spelling of the same remote as one repository", () => {
    const identity = "github.com/drferdii/sentrabot";
    expect(repoIdentity("https://github.com/drferdii/sentrabot")).toBe(identity);
    expect(repoIdentity("https://github.com/drferdii/sentrabot.git")).toBe(identity);
    expect(repoIdentity("git@github.com:drferdii/sentrabot.git")).toBe(identity);
    expect(repoIdentity("ssh://git@github.com/Drferdii/SentraBot")).toBe(identity);
    expect(repoIdentity("not a url")).toBeNull();
  });

  it("only calls the real upstream official", () => {
    expect(isOfficialRepoUrl("git@github.com:drferdii/sentrabot.git")).toBe(true);
    expect(isOfficialRepoUrl("https://github.com/attacker/sentrabot")).toBe(false);
    expect(isOfficialRepoUrl("https://githubb.com/drferdii/sentrabot")).toBe(false);
  });
});
...
describe("updateSteps", () => {
  ...
  it("passes every value as its own argument so nothing reaches a shell", () => {
    const steps = updateSteps({
      remoteUrl: "git@github.com:me/sentrabot.git",
      branch: "release/1.2",
      targetCommit: "2".repeat(40),
      repointRemote: true,
    });
    expect(steps[0]).toEqual({
      ...
      command: "git",
      args: ["remote", "set-url", "origin", "git@github.com:me/sentrabot.git"],
    });
