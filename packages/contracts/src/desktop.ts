...
export interface SentraBotDesktopUpdate {
...
export interface SentraBotDesktopOAuthCallback {
  code: string;
  state?: string;
}
...
export interface SentraBotDesktop {
  ...
  oauth: {
     ...
     */
    onCallback: (listener: (callback: SentraBotDesktopOAuthCallback) => void) => () => void;
  };
}

/** How the desktop app was pointed at a Sentra Bot server during first-run setup. */
 ...
 * narrower `sentrabotDesktop` bridge so a connected server can never re-point the app.
 */
export interface SentraBotSetup {
