...
import { SendBlueEmulator, SendBlueMessagingProvider } from "@sentrabot/adapters";
...
describePhone("phone surface journeys", () => {
  ...
  const dataDir = mkdtempSync(path.join(tmpdir(), "sentrabot-phone-"));
  ...
  it("ignores a nested vendor envelope that is not a flat inbound message", async () => {
    ...
    const res = await app.request("https://sentrabot.test/api/v1/phone/webhook", {
      method: "POST",
      headers: {
  ...
  it("reads registered groups through the nested vendor getGroup envelope", async () => {
    ...
    const { SendBlueMessagingProvider } = await import("@sentrabot/adapters");
