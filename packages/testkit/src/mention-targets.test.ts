...
describeWithDatabase("structured @ mention targets", () => {
  ...
  const dataDir = mkdtempSync(path.join(tmpdir(), "sentrabot-mention-targets-"));
  ...
  it("starts a routine test run on the owning bot", async () => {
    const cookie = await signup(app, `mention-routine-${stamp}@sentrabot.test`, "Routine Owner");
  ...
  it("lands a group-targeted send in the group transcript, not the 1:1 bot thread", async () => {
    const cookie = await signup(app, `mention-group-${stamp}@sentrabot.test`, "Group Owner");
  ...
  it("wakes a mentioned group member from typed bot chips and ignores non-members", async () => {
    const cookie = await signup(app, `mention-out-${stamp}@sentrabot.test`, "Out Of Chat");
    const botA = await rpc<{ id: string }>(app, cookie, "bots/create", {
      name: "MemberA",
      title: "",
      description: "",
  ...
  it("rejects another user's routine, connection, and group mentions", async () => {
    const ada = await signup(app, `mention-auth-ada-${stamp}@sentrabot.test`, "Ada Auth");
    const bob = await signup(app, `mention-auth-bob-${stamp}@sentrabot.test`, "Bob Auth");
