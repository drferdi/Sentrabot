...
import type { RunActivityRow } from "@sentrabot/contracts";
...
describeRunsList("runs.list activity tracker", () => {
  ...
  const dataDir = mkdtempSync(path.join(tmpdir(), "sentrabot-runs-list-"));
  ...
  it("lists active runs from two bots and keeps completed runs under recent", async () => {
    const cookie = await signup(app, `runs-${stamp}@sentrabot.test`, "Runs User");
  ...
  it("never returns runs from another workspace", async () => {
    const ownerCookie = await signup(app, `runs-owner-${stamp}@sentrabot.test`, "Owner");
    ...
    const intruderCookie = await signup(app, `runs-intruder-${stamp}@sentrabot.test`, "Intruder");
  ...
  it("excludes archived bots from recent before applying the limit", async () => {
    const cookie = await signup(app, `runs-archived-${stamp}@sentrabot.test`, "Archive User");
  ...
  it("returns group runs with groupId for navigation", async () => {
    const cookie = await signup(app, `runs-group-${stamp}@sentrabot.test`, "Group User");
