...
describeWithDatabase("Composio catalog reconciliation", () => {
  ...
  const dataDir = mkdtempSync(path.join(tmpdir(), "sentrabot-connections-"));
  ...
  it("reconciles one scoped row per provider under concurrent catalog fetches", async () => {
    const ownerCookie = await signup(app, `owner-connections-${stamp}@sentrabot.test`, "Owner");
    const otherCookie = await signup(app, `other-connections-${stamp}@sentrabot.test`, "Other");
  ...
  it("does not mutate local state when the provider catalog fails", async () => {
    const cookie = await signup(
      app,
      `provider-failure-connections-${stamp}@sentrabot.test`,
      "Provider Failure",
    );
    const actor = await rpc<Actor>(app, cookie, "me");
  ...
  it("routes an emulated Composio app tool with user-scoped connection context", async () => {
    const cookie = await signup(app, `composio-tool-${stamp}@sentrabot.test`, "Composio Tool");
    const actor = await rpc<Actor>(app, cookie, "me");
  ...
  it("runs Pipedream connection and MCP tool execution through the product registry", async () => {
    const cookie = await signup(app, `pipedream-${stamp}@sentrabot.test`, "Pipedream Connector");
    const actor = await rpc<Actor>(app, cookie, "me");
  ...
  it("installs Treg and custom MCP sources, discovers tools, and routes both calls", async () => {
    const cookie = await signup(app, `mcp-connectors-${stamp}@sentrabot.test`, "MCP Connectors");
    const actor = await rpc<Actor>(app, cookie, "me");
  ...
  it("imports an OpenAPI connector, keeps its credential encrypted, and routes calls", async () => {
    const cookie = await signup(app, `api-connector-${stamp}@sentrabot.test`, "API Connector");
    const actor = await rpc<Actor>(app, cookie, "me");
