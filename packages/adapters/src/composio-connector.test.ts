...
import type { AdapterContext, ConnectorEvent, ConnectorTool } from "@sentrabot/adapter-kit";
...
describe("composio tool mapping", () => {
  ...
  it("uses catalog-canonical toolkit slugs without preloading every tool", async () => {
    ...
    expect(
      ...
      { userId: "__sentrabot_catalog__", toolkits: undefined, sessionPreset: undefined },
  ...
  it("only fetches live Composio slugs when a Sentra Bot row is still pending or errored", () => {
  ...
  it("plans DB sync when Composio is connected but Sentra Bot is still pending", () => {
    expect(
      planLiveConnectionSync(
        [
