...
import { isLocalMcpHost } from "@sentrabot/contracts";
import type { McpServer, PrismaClient } from "@sentrabot/db";
...
export class McpConnector implements ConnectorProvider {
  ...
  private async connectSession(server: McpServer, context: AdapterContext): Promise<McpSession> {
    const session = new McpSession({ name: `sentrabot-${server.slug}` });
    try {
