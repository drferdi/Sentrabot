...
describe("searchSupermemory", () => {
  it("posts the query and container tag, returning search results on success", async () => {
    ...
    const result = await searchSupermemory("spelling preference", "sentrabot:bot-123", config);
    ...
    expect(JSON.parse(init.body)).toStrictEqual({
      ...
      containerTag: "sentrabot:bot-123",
      searchMode: "memories",
  ...
  it("reports a non-OK response instead of throwing", async () => {
    ...
    const result = await searchSupermemory("anything", "sentrabot:bot-123", config);
  ...
  it("treats document chunks as memory text when the search result has no memory field", async () => {
    ...
    const result = await searchSupermemory("database", "sentrabot:bot-123", config);
...
describe("searchSupermemoryContainers", () => {
  it("deduplicates and ranks results from shared and bot containers", async () => {
    ...
    await expect(
      searchSupermemoryContainers("query", ["sentrabot:workspace:ws-1", "sentrabot:bot-1"], config),
    ).resolves.toEqual({
      ok: true,
      results: [
        { memory: "private", similarity: 0.9 },
...
describe("saveSupermemoryMemory", () => {
  ...
  it("reports a non-OK response instead of throwing", async () => {
    ...
    const result = await saveSupermemoryMemory("fact", "sentrabot:bot-123", config);
...
describe("deleteSupermemoryContainer", () => {
  it("deletes the container tag on success", async () => {
    ...
    const result = await deleteSupermemoryContainer("sentrabot:bot-123", config);

    expect(result).toEqual({ ok: true });
    const [url, init] = fetchMock.mock.calls[0]!;
    expect(url).toBe("http://localhost:6767/v3/container-tags/sentrabot%3Abot-123");
