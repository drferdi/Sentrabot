...
function catalogBilling(
  ...
  if (opts.oauth && !opts.apiKey) {
    return `${name} subscription login is not in the Sentra Agent UI yet. Skip if this deployment already has credentials.`;
  }
  if (opts.apiKey) {
    return `Uses your ${name} API key. Sentra Agent does not pay for model usage.`;
  }
  return `Uses your ${name} key. Sentra Agent does not pay for model usage.`;
}
