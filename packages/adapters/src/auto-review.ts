import type { AgentModelOAuthCredential, AgentRuntime } from "@sentrabot/adapter-kit";
import type { ActionApprovalRule } from "@sentrabot/core";
import { type AutoReviewJudgeDecision, redactSecrets } from "@sentrabot/core";
...
function localModelIds(env: NodeJS.ProcessEnv): string[] {
  return (env.SENTRABOT_LOCAL_MODELS ?? "")
    .split(",")
    .map((id) => id.trim())
...
export function deploymentAutoReviewDefault(env: NodeJS.ProcessEnv = process.env): boolean {
  return envFlag(env, "SENTRABOT_AUTO_REVIEW");
}

export function autoReviewTimeoutMs(env: NodeJS.ProcessEnv = process.env): number {
  const raw = env.SENTRABOT_AUTO_REVIEW_TIMEOUT_MS?.trim();
...
export function resolveAutoReviewChecker(
  ...
  const overrideProvider = env.SENTRABOT_AUTO_REVIEW_PROVIDER?.trim();
  const overrideModel = env.SENTRABOT_AUTO_REVIEW_MODEL?.trim();
