...
import type { MessageBlock, RunStatus } from "@sentrabot/contracts";
import { ATTACHMENT_MAX_BYTES, isAttachmentImageMimeType } from "@sentrabot/contracts";
import {
...
const STATIC_SHELL_EXPANSIONS: Readonly<Record<string, string>> = {
  ...
  TMPDIR: "/tmp",
  USER: "sentrabot",
  WORKSPACE: "/home/sentrabot/workspace",
  XDG_CONFIG_HOME: "/home/sentrabot/.config",
};
...
export function buildApprovalContinuation(
  ...
  return [
    "Sentra Bot is resuming after the user approved the exact tool request(s) below.",
...
async function executeRunAttempt({
  ...
  workerId: string;
  fence: number;
  attempt: Awaited<ReturnType<PrismaClient["attempt"]["create"]>>;
  computerLease: ComputerExecutionLease;
  ...
  try {
    ...
    const graphical = computer.kind !== "desktop" && deps.sandbox.describe().capabilities.graphical;
    ...
    const applyTool = async (name: string, args: Record<string, unknown>, executionId: string) => {
      ...
      const claimOrReturn = async (from: "approved" | "intended"): Promise<unknown | undefined> => {
          ...
          const created = await deps.prisma.$transaction(async (tx) => {
            ...
            const server = await tx.mcpServer.create({
              data: {
                ...
                headers: Object.fromEntries(Object.keys(parsed.headers).map((key) => [key, true])),
          ...
          return commitConsumedRunSecret({
            ...
            persist: (secretResult) =>
              applied?.duplicate && applied.effect.status === "completed"
                ? replaceCompletedExternalEffectResult(deps.prisma, applied.effect.id, secretResult)
      ...
      if (name === "message_bot") {
        const sent = await messageBot(
          ...
          { id: bot.id, name: bot.name },
          {
            ...
            intent: args.intent as "request" | "result" | "question" | "status" | "fyi" | undefined,
      ...
      if (name === "archive_bot" || name === "delete_bot") {
        const archived = await archiveSpawnedBot(
          deps,
          {
            ...
            botId: args.bot_id ? String(args.bot_id) : args.botId ? String(args.botId) : undefined,
          },
          context,
        );
    ...
    try {
      for await (const event of deps.runtime.run(
        ...
        if (event.type === "text") {
          ...
          const safeDetail = event.detail ? redactSecrets(event.detail, runSecrets) : event.detail;
          await checkpointAndRecordComputerWorkspace(deps, storedComputer, computer, context);
          const paused = await deps.events.pauseRunForInput({
            workspaceId: run.workspaceId,
            threadId: run.threadId,
      ...
      if (!terminalCheckpointComplete) {
        await checkpointAndRecordComputerWorkspace(deps, storedComputer, computer, context).catch(
          () => undefined,
        );
      }
...
async function loadCurrentTurnImages(
  ...
  const images: NonNullable<import("@sentrabot/adapter-kit").AgentRunRequest["currentTurnImages"]> =
    [];
