#!/usr/bin/env python3
"""Token-auth desktop control for the Sentra Bot supervisor."""
...
TOKEN = os.environ.get("SENTRABOT_COMPUTER_CONTROL_TOKEN", "")
...
class NativeCapture:
    ...
    def __init__(self, display):
        library = ctypes.CDLL("/usr/local/lib/libsentrabot-xcapture.so")
        library.sentrabot_xcapture_open.argtypes = [ctypes.c_char_p]
        library.sentrabot_xcapture_open.restype = ctypes.c_void_p
        library.sentrabot_xcapture_png.argtypes = [
            ctypes.c_void_p,
        ...
        library.sentrabot_xcapture_png.restype = ctypes.c_int
        library.sentrabot_xcapture_damage.argtypes = [
            ctypes.c_void_p,
            ctypes.POINTER(ctypes.c_int),
            ctypes.POINTER(ctypes.c_int),
            ctypes.POINTER(ctypes.c_int),
        ...
        library.sentrabot_xcapture_damage.restype = ctypes.c_int
        library.sentrabot_xinput_argv.argtypes = [ctypes.c_void_p, ctypes.c_int, ctypes.POINTER(ctypes.c_char_p)]
        library.sentrabot_xinput_argv.restype = ctypes.c_int
        context = library.sentrabot_xcapture_open(display.encode("utf-8"))
        if not context:
    ...
    def copy(self):
        ...
        if self.library.sentrabot_xcapture_png(
        ...
        changed = self.library.sentrabot_xcapture_damage(
    ...
    def act(self, argv):
        ...
        return self.library.sentrabot_xinput_argv(self.context, len(argv), encoded)
