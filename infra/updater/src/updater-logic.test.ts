import { DEFAULT_COMPOSE_PROJECT_NAME, isLocalImageTag } from "@sentrabot/core";
...
const base = {
  ...
  SENTRABOT_UPDATER_TOKEN: "fake-review-updater-token-000000000000",
} as const;
...
describe("resolveUpdaterConfig", () => {
  it("derives the compose file, env file, and defaults from the deployment directory", () => {
    ...
    expect(config).toMatchObject({
      deployDir: "/srv/sentrabot",
      composeFile: `/srv/sentrabot/${DEFAULT_COMPOSE_FILE}`,
      envFile: "/srv/sentrabot/.env",
      projectName: DEFAULT_COMPOSE_PROJECT_NAME,
      token: base.SENTRABOT_UPDATER_TOKEN,
  ...
  it("uses the Compose-injected project name so -p matches the running stack", () => {
    ...
    expect(
      resolveUpdaterConfig({ ...base, SENTRABOT_COMPOSE_PROJECT_NAME: "manual-stack" }).projectName,
    ).toBe("manual-stack");
  });
  ...
  it("refuses a compose path that escapes the deployment directory", () => {
    for (const composeFile of ["/etc/compose.yml", "../../etc/compose.yml", "a/../../b.yml"]) {
      expect(() => resolveUpdaterConfig({ ...base, SENTRABOT_COMPOSE_FILE: composeFile })).toThrow(
        /SENTRABOT_COMPOSE_FILE/,
      );
    }
  });

  it("refuses an image name it would not be willing to hand to compose", () => {
    expect(() => resolveUpdaterConfig({ ...base, SENTRABOT_IMAGE: "Bad Name" })).toThrow(
      /SENTRABOT_IMAGE/,
    );
    expect(resolveUpdaterConfig({ ...base, SENTRABOT_IMAGE: "ghcr.io/me/app" }).image).toBe(
      "ghcr.io/me/app",
    );
  });
...
describe("readEnvAssignment", () => {
  it("reads the last assignment, ignoring comments and blank lines", () => {
    const contents = ["# SENTRABOT_IMAGE_TAG=commented", "", "A=1", "A=2"].join("\n");
    expect(readEnvAssignment(contents, "A")).toBe("2");
    expect(readEnvAssignment(contents, "SENTRABOT_IMAGE_TAG")).toBeNull();
  });
  ...
  it("does not match a key that merely shares a prefix", () => {
    expect(readEnvAssignment("SENTRABOT_IMAGE_TAG_PREVIOUS=v1", "SENTRABOT_IMAGE_TAG")).toBeNull();
  });
});

describe("readTagState", () => {
  it("reads the pinned tag and the rollback tag", () => {
    const contents = "SENTRABOT_IMAGE_TAG=v1.1.0\nSENTRABOT_IMAGE_TAG_PREVIOUS=v1.0.0\n";
  ...
  it("ignores values in the file that are not usable tags", () => {
    const contents = "SENTRABOT_IMAGE_TAG=-rm\nSENTRABOT_IMAGE_TAG_PREVIOUS=$(id)\n";
