...
describe("the images compose file", () => {
  it("runs postgres, app roles, supervisor, and a published computer image", () => {
    ...
    for (const service of appServices) {
      expect(compose.services[service]?.image).toContain("ghcr.io/drferdii/sentrabot/app");
      expect(compose.services[service]?.image).toContain("SENTRABOT_IMAGE_TAG");
    }
    expect(compose.services.computer?.image).toContain("ghcr.io/drferdii/sentrabot/computer");
    expect(compose.services.computer?.image).toContain("SENTRABOT_COMPUTER_IMAGE_TAG");
