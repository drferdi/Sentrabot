...
import type { ServerUpdateRun } from "@sentrabot/contracts";
...
const app = createUpdaterApp(
  resolveUpdaterConfig({
    SENTRABOT_DEPLOY_DIR: "/sentrabot-updater-tests-no-such-directory",
...
async function deployment(
  ...
  // Node's fs APIs resolve a leading "/" against the current drive on Windows and accept forward
  // slashes on every platform, so the same path string is valid for both the real fs calls below and
  // for resolveUpdaterConfig's POSIX-absolute check.
  await mkdir("/tmp", { recursive: true });
  const deployDir = await mkdtemp("/tmp/sentrabot-updater-test-");
...
describe("updater HTTP surface", () => {
  ...
  it("refuses a fork build when the deployment has no checkout to build from", async () => {
    ...
    const response = await createUpdaterApp(fixture.config).request("/apply", {
      method: "POST",
      headers: authorized,
      body: JSON.stringify({ repoUrl: "https://github.com/someone/sentrabot", branch: "main" }),
    });
    expect(response.status).toBe(400);
    const payload = (await response.json()) as { error: string };
  ...
  it("refuses a rollback when no previous tag was recorded", async () => {
    const fixture = await deployment("SENTRABOT_IMAGE_TAG=v1.0.0\n");
...
describe("updater orchestration", () => {
  it("serializes updates before their asynchronous preflight can race", async () => {
    ...
    const input = { repoUrl: "https://github.com/drferdii/sentrabot", branch: "main" };
  ...
  it("restores the prior image after a recreate fails health checks", async () => {
    ...
    expect(composeUp[0]?.env?.SENTRABOT_IMAGE_TAG).toBe(`sha-${targetCommit}`);
    expect(composeUp[1]?.env?.SENTRABOT_IMAGE_TAG).toBe("v1.0.0");
    expect(await readFile(path.join(fixture.deployDir, ".env"), "utf8")).toContain(
      "SENTRABOT_IMAGE_TAG=v1.0.0",
    );
  ...
  it("resets a fork checkout when recreate fails after the fast-forward", async () => {
    const fixture = await deployment("SENTRABOT_IMAGE_TAG=local\n");
    await mkdir(path.join(fixture.deployDir, ".git"));
    const calls: string[][] = [];
    let upCalls = 0;
    const run: UpdaterCommandRunner = async (command, args) => {
  ...
  // Windows/NTFS has no POSIX permission bits — chmod() there only toggles the read-only attribute,
  // so a "writable" mode always collapses to 0o666 regardless of what was requested. The 0o600
  // behavior this test asserts is real and enforced in production (Linux containers only); it is
  // simply not observable through chmod()/lstat() on this platform.
  it.skipIf(process.platform === "win32")(
    "preserves the environment owner and surrounding values with mode 0600",
    async () => {
  ...
  it("refuses to replace a symlinked deployment environment", async () => {
    ...
    await writeFile(target, "SENTRABOT_IMAGE_TAG=v1.0.0\n");
    await rm(envFile);
    ...
    expect(await readFile(target, "utf8")).toBe("SENTRABOT_IMAGE_TAG=v1.0.0\n");
  });
