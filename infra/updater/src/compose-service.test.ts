...
import { RECREATED_SERVICES } from "@sentrabot/core";
...
describe("the updater compose service", () => {
  ...
  it("is bind-mounted at the same path it has on the host", () => {
    const mount = (updater.volumes ?? []).find((volume) => volume.includes("SENTRABOT_DEPLOY_DIR"));
    // biome-ignore lint/suspicious/noTemplateCurlyInString: this is the literal Compose expression
    const deployDir = "${SENTRABOT_DEPLOY_DIR:-/srv/sentrabot}";
    ...
    expect(updater.environment?.SENTRABOT_DEPLOY_DIR).toBe(deployDir);
  ...
  it("pins its own image tag separately from the application image", () => {
    expect(updater.image).toContain("SENTRABOT_UPDATER_IMAGE_TAG");
    for (const service of RECREATED_SERVICES) {
      expect(compose.services[service]?.image).toContain("SENTRABOT_IMAGE_TAG");
    }
  });

  it("uses the official registry namespace and digest-pins third-party runtime images", () => {
    expect(updater.image).toContain("ghcr.io/drferdii/sentrabot/updater");
    expect(compose.services.api?.image).toContain("ghcr.io/drferdii/sentrabot/app");
  ...
  it("does not load the application env_file into the root-equivalent process", () => {
    ...
    // biome-ignore lint/suspicious/noTemplateCurlyInString: this is the literal Compose expression
    expect(updater.environment?.SENTRABOT_UPDATER_TOKEN).toBe("${SENTRABOT_UPDATER_TOKEN:-}");
  });
  ...
  it("does not let the api container reach the Docker socket to update itself", () => {
    ...
    expect(compose.services.api?.environment?.SENTRABOT_UPDATER_URL).toBe("http://updater:7092");
  });
});
