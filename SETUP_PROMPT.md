# Set up Sentra Bot with a coding agent
...
```text
Set up Sentra Bot from published GHCR images and leave the web UI running.
...
1. Which directory should contain the Sentra Bot folder (or use the current directory)?
2. How should models be connected?
   - Add a deployment-wide `OPENROUTER_API_KEY` to `.env`.
   - Connect during Sentra Bot onboarding with a provider API key or with ChatGPT Plus/Pro, GitHub Copilot, or SuperGrok / X Premium.
...
2. Download and inspect this installer (do not clone the repository):
   https://raw.githubusercontent.com/drferdii/sentrabot/main/infra/compose/install-images.sh
...
```text
Set up Sentra Bot locally and leave it running in a usable state.

Repository: https://github.com/drferdii/sentrabot.git
...
1. Should you clone into the current directory, or what parent directory should contain `sentrabot`? If you are already inside a Sentra Bot checkout, offer to use it without recloning.
2. How should models be connected?
   - Add a deployment-wide `OPENROUTER_API_KEY` to `.env`.
   - Connect during Sentra Bot onboarding with a provider API key or with ChatGPT Plus/Pro, GitHub Copilot, or SuperGrok / X Premium.
   - Defer model setup and verify infrastructure only. Make clear that bots cannot answer until a model is connected.
...
- If I requested Electron, leave the web stack running and then launch `pnpm --filter @sentrabot/desktop dev`. Verify the shell loads the same app. Let me make the Docker-versus-This-Mac choice because This Mac grants bots access under my OS account.
